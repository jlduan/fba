...
def add_filter_subparser(subparsers):
    parser = subparsers.add_parser(
        ...
        description='Filter extracted cell and feature barcoding results (output of `extract` or `qc`). Additional fragment selection can be performed through `-cb_seq` and/or `-fb_seq`.')
...
def add_demultiplex_subparser(subparsers):
    parser = subparsers.add_parser(
        ...
        description='Demultiplex cells based on the abundance of features (matrix generated by `count`).'
    )
...
def add_qc_subparser(subparsers):
    parser = subparsers.add_parser(
        ...
        description='Generate diagnostic results through fastq files. If `-1` is omitted, bulk mode is enabled and only reads 2 will be analyzed.'
    )
